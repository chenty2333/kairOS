...
- GitHub Actions `ci-flake-report` runs daily (and on manual dispatch) to compute rolling failure-rate stats for key gates in `ci-quick`/`soak-long` over the latest N completed runs per workflow (default `20`); the report is generated by `scripts/impl/ci-flake-report.py` and published as both step summary and downloadable artifact (`ci-flake-report.json` / `ci-flake-report.md`), with both job-level and selected step-level rows (for example `x86_64` quick sub-gates and `aarch64` directed soak loops)
