...
- GitHub Actions `ci-quick` runs per-arch gate jobs: `riscv64-test` (`syscall-trap`, `vfs/ipc`, `driver`, `socket`, `tcc-smoke`, `abi-smoke`), `x86_64-gates` (same + `test-x86-boot-smp`), `aarch64-smoke` (`smp2` full quick set + `smp4` `syscall-trap`/`vfs-ipc`), and `x86_64-irq-soak` (`test-irq-soak`); `riscv64-test` also publishes boot diagnostics artifacts (`qemu-version`, firmware hash, `qemu-virt-ci.dtb`, repro `boot.img` bundle) for external triage.
...
- GitHub Actions `ci-flake-report` runs on PRs, pushes to `main`/`master`, daily schedule, and manual dispatch; it computes rolling failure-rate stats for key gates in `ci-quick`/`soak-long` over the latest N completed runs per workflow (default `20`) and publishes both step summary + artifacts (`ci-flake-report.json` / `ci-flake-report.md`); flake-budget enforcement (`--max-fail-rate-percent`, default `10`; `--min-evaluated`, default `10`) is enabled on non-PR runs and disabled on PR runs (report-only mode).
