...
- GitHub Actions `ci-quick` also runs `aarch64-smp4-shadow` as a non-blocking (`continue-on-error`) lane to collect `QEMU_SMP=4` stability data across syscall/vfs-ipc/driver/socket/tcc paths with `assert-aarch64-smp.py --expected-online 4`
