...
- GitHub Actions `ci-quick` runs on PRs, pushes to `main`/`master`, and `quick-*` tags; it runs `riscv64` default regression gate (`make test-ci-default`), `x86_64` blocking gates (`make test-syscall-trap` + `make test-vfs-ipc` + `make test-driver` + `make test-socket` + `make test-tcc-smoke`), and `aarch64` SMP=2 gates (`make QEMU_SMP=2 test-syscall-trap` + `make QEMU_SMP=2 test-vfs-ipc` + `make QEMU_SMP=2 test-driver` + `make QEMU_SMP=2 test-socket` + `make QEMU_SMP=2 test-tcc-smoke`); on `riscv64`, `test-ci-default` uses `TEST_TIMEOUT=420` for quick regression and skips interactive smoke subtargets; all `aarch64` SMP=2 gates validate stability via `scripts/impl/assert-aarch64-smp.py` (require `SMP: 2/2 CPUs active`, reject shortfall/stall diagnostics)
- GitHub Actions `ci-quick` also runs blocking `aarch64` SMP=4 gates for syscall/vfs-ipc/driver/socket/tcc paths (`make QEMU_SMP=4 test-syscall-trap` + `make QEMU_SMP=4 test-vfs-ipc` + `make QEMU_SMP=4 test-driver` + `make QEMU_SMP=4 test-socket` + `make QEMU_SMP=4 test-tcc-smoke`); all SMP=4 gates validate stability via `scripts/impl/assert-aarch64-smp.py --expected-online 4`
- GitHub Actions `soak-long` runs `riscv64` long soak-pr profile plus `x86_64` and `aarch64` bootstrap soak-pr profiles (shorter default duration and timeout for CI cost control); before `aarch64` soak-pr mixed-suite runs, it executes a directed `QEMU_SMP=2 test-vfs-ipc` loop profile (`AARCH64_VFS_IPC_LOOP_ROUNDS`, default 6) and applies the same SMP stability assertion (`scripts/impl/assert-aarch64-smp.py`)
