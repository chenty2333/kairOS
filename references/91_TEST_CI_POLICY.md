...
- GitHub Actions `ci-quick` runs on PRs, pushes to `main`/`master`, and `quick-*` tags; it runs blocking per-suite gates on all three architectures: `riscv64` (`make test-syscall-trap` + `make test-vfs-ipc` + `make test-driver` + `make test-socket` + `make test-tcc-smoke`), `x86_64` (same five gates), and `aarch64` SMP=2 (same five gates with `QEMU_SMP=2`); each quick sub-gate validates isolated structured results via `scripts/impl/assert-result-pass.py --require-structured`, and all `aarch64` SMP=2 gates additionally validate CPU-online stability via `scripts/impl/assert-aarch64-smp.py` (require `SMP: 2/2 CPUs active`, reject shortfall/stall diagnostics)
