...
- GitHub Actions `ci-quick` runs on PRs, pushes to `main`/`master`, and `quick-*` tags; it runs blocking per-suite gates on all three architectures: `riscv64` (`make test-syscall-trap` + `make test-vfs-ipc` + `make test-driver` + `make test-socket` + `make test-device-virtio` + `make test-tcc-smoke`), `x86_64` (same five structured gates + `make test-x86-boot-smp`), and `aarch64` SMP=2 (same five gates with `QEMU_SMP=2`); structured result assertions (`scripts/impl/assert-result-pass.py --require-structured`) are applied to the isolated kernel-test/tcc gates, and all `aarch64` SMP=2 gates additionally validate CPU-online stability via `scripts/impl/assert-aarch64-smp.py` (require `SMP: 2/2 CPUs active`, reject shortfall/stall diagnostics)
