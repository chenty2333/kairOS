...
- GitHub Actions `ci-quick` runs on PRs, pushes to `main`/`master`, and `quick-*` tags; it runs `riscv64` default regression gate (`make test-ci-default`), `x86_64` minimal smoke (`make test-driver`), and `aarch64` SMP=2 gates (`make QEMU_SMP=2 test-syscall-trap` + `make QEMU_SMP=2 test-vfs-ipc` + `make QEMU_SMP=2 test-driver` + `make QEMU_SMP=2 test-socket` + `make QEMU_SMP=2 test-tcc-smoke`); on `riscv64`, `test-ci-default` uses `TEST_TIMEOUT=420` for quick regression and skips interactive smoke subtargets; all `aarch64` SMP=2 gates validate stability via `scripts/impl/assert-aarch64-smp.py` (require `SMP: 2/2 CPUs active`, reject shortfall/stall diagnostics)
