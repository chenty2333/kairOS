...
- GitHub Actions `ci-quick` runs on PRs, pushes to `main`/`master`, and `quick-*` tags; it runs blocking per-suite gates on all three architectures: `riscv64` (`make test-syscall-trap` + `make test-vfs-ipc` + `make test-driver` + `make test-socket` + `make test-device-virtio` + `make test-tcc-smoke`), `x86_64` (same five gates), and `aarch64` SMP=2 (same five gates with `QEMU_SMP=2`); structured result assertions (`scripts/impl/assert-result-pass.py --require-structured`) are applied to the isolated kernel-test/tcc gates, and all `aarch64` SMP=2 gates additionally validate CPU-online stability via `scripts/impl/assert-aarch64-smp.py` (require `SMP: 2/2 CPUs active`, reject shortfall/stall diagnostics)
- GitHub Actions `ci-quick` keeps only one `aarch64` job (`aarch64-smoke`); besides the blocking SMP=2 gate set (`test-syscall-trap` + `test-vfs-ipc` + `test-driver` + `test-socket` + `test-tcc-smoke`), it retains SMP=4 sanity probes for syscall/vfs-ipc (`make QEMU_SMP=4 test-syscall-trap` + `make QEMU_SMP=4 test-vfs-ipc`), all with `scripts/impl/assert-aarch64-smp.py` assertions
- GitHub Actions `soak-long` runs `riscv64` long soak-pr profile plus `x86_64` and `aarch64` bootstrap soak-pr profiles (shorter default duration and timeout for CI cost control); before `aarch64` soak-pr mixed-suite runs, it executes directed `QEMU_SMP=4` loop profiles for `test-vfs-ipc` (`AARCH64_VFS_IPC_LOOP_ROUNDS`, default 6), `test-socket` (`AARCH64_SOCKET_LOOP_ROUNDS`, default 4), and `test-driver` (`AARCH64_DRIVER_LOOP_ROUNDS`, default 4), and applies the same SMP stability assertion (`scripts/impl/assert-aarch64-smp.py --expected-online 4`)
- GitHub Actions `ci-flake-report` runs on PRs, pushes to `main`/`master`, daily schedule, and manual dispatch; it computes rolling failure-rate stats for key gates in `ci-quick`/`soak-long` over the latest N completed runs per workflow (default `20`) and enforces a flake budget threshold (`--max-fail-rate-percent`, default `10`; `--min-evaluated`, default `10`) by failing when any enforced gate exceeds the configured fail-rate cap; the report is generated by `scripts/impl/ci-flake-report.py` and published as both step summary and downloadable artifact (`ci-flake-report.json` / `ci-flake-report.md`), with job-level plus selected step-level rows (for example `riscv64`/`x86_64` quick sub-gates and `aarch64` directed soak loops)
