...
- sysfs exports IPC observability at `/sys/ipc`: aggregated views are `/sys/ipc/{channels,ports,transfers}`, and `/sys/ipc/objects/` exposes v2 paging controls (`page`, `cursor`, `page_size`) plus per-object dirs (`/sys/ipc/objects/<id>/{summary,transfers,transfers_v2,transfers_cursor,transfers_page_size}`) for registered channel/port objects
