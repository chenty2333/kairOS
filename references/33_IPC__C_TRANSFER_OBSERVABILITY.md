...
- `TRACE_IPC_CAP` exposes capability lineage race/guard events (`revoke_marked`, `bind_rejected_revoked`, `commit_eagain`, `tryget_failed`, `commit_epoch_mismatch`) with versioned flags (`op[7:0]`, `version[31:28]`); `/sys/kernel/tracepoint/ipc_cap_events` exports decoded `trace_ipc_cap_v1` rows (`op/cap_id/arg1`), where `commit_epoch_mismatch` packs `arg1=(expected_low32<<32)|current_low32` for offline decoding
...
- latest riscv64 focused verification snapshot:
  - `test-socket` pass (`RUN_ID=fix-timeout-socket-102231`), `ipc_registry_id`: `buckets=1024 entries=54 collision_entries=4 max_bucket_depth=2`
  - `test-vfs-ipc` pass (`RUN_ID=fix-timeout-vfsipc-102435`), `ipc_registry_id`: `buckets=1024 entries=996 collision_entries=365 max_bucket_depth=7 rehash_recommended=0`
  - `test-ipc-cap` pass (`RUN_ID=epoch-mismatch-fix-ipccap-104101`), `ipc_registry_id`: `buckets=1024 entries=39 collision_entries=0 max_bucket_depth=1 rehash_recommended=0`; `khandle_timeout reclaimed slot` no longer reproduces, and commit epoch drift is now tracked via `/sys/ipc/stats:cap_commit_epoch_mismatch_total` without warning-log spam
