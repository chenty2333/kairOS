...
int64_t sys_chroot(uint64_t path_ptr, uint64_t a1, uint64_t a2, uint64_t a3,
    ...
    int ret = build_at_path(AT_FDCWD, kpath, full);
    if (ret < 0)
        return ret;

    struct path resolved;
    path_init(&resolved);
    ret = vfs_namei(full, &resolved, NAMEI_FOLLOW | NAMEI_DIRECTORY);
    if (ret < 0)
        return ret;
    if (!resolved.dentry || !resolved.dentry->vnode) {
        if (resolved.dentry)
...
int64_t sys_pivot_root(uint64_t new_root_ptr, uint64_t put_old_ptr,
    ...
    char new_full[CONFIG_PATH_MAX];
    char old_full[CONFIG_PATH_MAX];
    int ret = build_at_path(AT_FDCWD, new_root, new_full);
    if (ret < 0)
        return ret;
    ret = build_at_path(AT_FDCWD, put_old, old_full);
    if (ret < 0)
        return ret;
    ...
    path_init(&newp);
    path_init(&oldp);
    ret = vfs_namei(new_full, &newp, NAMEI_FOLLOW | NAMEI_DIRECTORY);
    if (ret < 0)
        return ret;
    ret = vfs_namei(old_full, &oldp, NAMEI_FOLLOW | NAMEI_DIRECTORY);
    if (ret < 0) {
        if (newp.dentry)
...
int64_t sys_mount(uint64_t source_ptr, uint64_t target_ptr, uint64_t fstype_ptr,
    ...
    int ret = build_at_path(AT_FDCWD, target, target_full);
    if (ret < 0)
        return ret;
    ...
    if (flags & prop_mask) {
        struct path tpath;
        path_init(&tpath);
        ret = vfs_namei(target_full, &tpath, NAMEI_FOLLOW | NAMEI_DIRECTORY);
        if (ret < 0)
            return ret;
    ...
    if (flags & MS_BIND) {
        ...
        ret = build_at_path(AT_FDCWD, source, source_full);
        if (ret < 0)
            return ret;
        struct path spath, tpath;
        path_init(&spath);
        path_init(&tpath);
        ret = vfs_namei(source_full, &spath, NAMEI_FOLLOW | NAMEI_DIRECTORY);
        if (ret < 0)
            return ret;
        ret = vfs_namei(target_full, &tpath, NAMEI_FOLLOW | NAMEI_DIRECTORY);
        if (ret < 0) {
    ...
    if (!fstype_ptr)
        return -EINVAL;
    return vfs_mount(source_ptr ? source : NULL, target_full, fstype,
                     (uint32_t)flags);
}
