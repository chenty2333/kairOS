    ...
    if (ctx && ctx->magic == CHANFD_MAGIC) {
        ...
        if (ctx->channel_obj) {
            if (ctx->endpoint_ref_held) {
                kchannel_endpoint_ref_dec_owner(
...
static int channelfd_create_file(struct kobj *obj, uint32_t rights,
    ...
    *out = NULL;
    ...
    ctx->endpoint_ref_held = false;
    kobj_get(obj);
    kchannel_endpoint_ref_inc_owner(obj,
                                    KCHANNEL_ENDPOINT_REF_OWNER_CHANNEL_FD);
    ...
    if (rc < 0) {
        if (ctx->endpoint_ref_held) {
            kchannel_endpoint_ref_dec_owner(
                obj, KCHANNEL_ENDPOINT_REF_OWNER_CHANNEL_FD);
            ctx->endpoint_ref_held = false;
        }
        kobj_put(obj);
        vfs_file_free(file);
...
int64_t sys_kairos_channel_send(uint64_t handle, uint64_t msg_ptr,
    ...
    for (; taken < msg.num_handles; taken++) {
        rc = khandle_reserve_transfer(
            ...
            &transfers[taken].slot_generation);
        if (rc < 0)
            break;
    }
    if (rc < 0) {
        int rollback_rc = syshandle_abort_reserved(p, user_handles, transfers, taken);
    ...
    rc = kchannel_send(channel_obj, bytes, msg.num_bytes, transfers, msg.num_handles,
                       (uint32_t)options);
    if (rc < 0) {
        int rollback_rc = syshandle_abort_reserved(
            p, user_handles, transfers, msg.num_handles);
        kobj_put(channel_obj);
        if (rollback_rc < 0)
            return rollback_rc;
        return rc;
...
int64_t sys_kairos_channel_recv(uint64_t handle, uint64_t msg_ptr,
    ...
    for (size_t i = 0; i < got_handles; i++) {
        int32_t installed_handle = -1;
        rc = khandle_install_transferred_cap(
            p, transfers[i].obj, transfers[i].rights, transfers[i].cap_id,
            &installed_handle);
        if (rc < 0) {
            syshandle_close_installed(p, installed, i);
            for (size_t j = i; j < got_handles; j++) {
                if (transfers[j].obj)
                    khandle_transfer_drop_cap(transfers[j].obj,
                                              transfers[j].rights,
                                              transfers[j].cap_id);
            }
            kobj_put(channel_obj);
